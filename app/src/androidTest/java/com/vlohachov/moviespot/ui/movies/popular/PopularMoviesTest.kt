...
class PopularMoviesTest {
    ...
    private val viewModel = mockk<PopularMoviesViewModel>()
    ...
    fun titleTest(): Unit = with(composeRule) {
        ...
        setContent {
            MoviesPotTheme {
                PopularMovies(navigator = navigator, viewModel = viewModel)
            }
        }

        onNodeWithText(text = context.getString(R.string.popular), useUnmergedTree = true)
    ...
    fun scrollToTopTest(): Unit = with(composeRule) {
        ...
        onNodeWithTag(testTag = PopularMoviesDefaults.ScrollToTopTestTag, useUnmergedTree = true)
